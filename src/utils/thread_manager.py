...
class ThreadResourceManager:
    ...
    def can_create_thread(cls, is_background: bool = False, component_id: str = None) -> bool:
        ...
        with cls._thread_lock:
            ...
            if is_background and cls._background_threads >= cls.MAX_BACKGROUND_THREADS:
                logger.warning(f"Background thread creation denied: reached max background threads ({cls.MAX_BACKGROUND_THREADS})")
            ...
            if component_id:
                ...
                if cls._component_threads[component_id] >= cls.MAX_THREADS_PER_COMPONENT:
                    logger.warning(f"Thread creation denied: component {component_id} reached limit ({cls.MAX_THREADS_PER_COMPONENT})")
        ...
        with cls._thread_lock:
            if thread_id in cls._thread_registry:
                ...
                if runtime > cls.THREAD_TIMEOUT_SECONDS:
                    logger.warning(f"Thread {thread_id} ran for {runtime:.1f}s (timeout: {cls.THREAD_TIMEOUT_SECONDS}s)")
    ...
    def _monitor_thread_resources(cls, thread_id: str):
        ...
        try:
            ...
            thread_info = cls._thread_registry.get(thread_id, {})
