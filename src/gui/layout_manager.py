    ...
    def select_layout(self, screen_width: int, screen_height: int, diagonal: float) -> Optional[ScreenLayout]:
        ...
        if best_layout is None:
            best_layout = min(self.SCREEN_LAYOUTS, 
                            key=lambda l: abs(l.resolution[0] - screen_width) + abs(l.resolution[1] - screen_height))
                    
        logger.info(f"Selected layout: {best_layout.diagonal}\" ({best_layout.resolution[0]}x{best_layout.resolution[1]})")
        return best_layout
